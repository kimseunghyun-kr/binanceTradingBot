...
stateDiagram-v2
    ...
    Position_Open --> Position_Open: TP/SL 필수, 변경불가, 부분청산 불가
```
...
### 3.1 핵심 기능
- **롱/숏 전략**: 양방향 포지션 지원
- **포트폴리오 리밸런싱**: 주기적 비중 조정
- **페어 트레이딩**: 상관 자산 간 스프레드 거래
- **리스크 패리티**: 변동성 기반 자산 배분
- **팩터 투자**: 모멘텀, 밸류, 퀄리티 등 팩터 기반 전략

### 3.2 리스크 관리
- **동적 헤징**: 시장 상황에 따른 헤지 비율 조정
- **포지션 사이징**: 켈리 공식, 리스크 패리티 등
- **상관관계 분석**: 포트폴리오 분산 효과 극대화
...
stateDiagram-v2
    ...
    Multi_Signal --> Multi_Signal: 다중시그널, 롱숏동시, 커스텀Exit
```
...
### Phase 1: 기반 구조 개선 (4주)
- **주 1-2**: TradeEventType 확장 및 ExitStrategy 시스템 구현
- **주 3-4**: AdvancedBaseStrategy 및 AdvancedPortfolioManager 구현

### Phase 2: 롱숏 기능 구현 (3주)
- **주 5-6**: SHORT 포지션 지원 추가
- **주 7**: 롱숏 동시 보유 및 넷 포지션 관리

### Phase 3: 리밸런싱 기능 (3주)
- **주 8-9**: 부분 청산 및 포지션 조정 기능
- **주 10**: 시간/조건 기반 리밸런싱 트리거

### Phase 4: 고급 전략 구현 (4주)
- **주 11-12**: 마켓 뉴트럴, 페어 트레이딩 전략
- **주 13-14**: 팩터 기반 전략 및 리스크 패리티

### Phase 5: 테스트 및 최적화 (2주)
- **주 15**: 통합 테스트 및 성능 최적화
- **주 16**: 문서화 및 배포 준비
...
- **전략 다양성**: 단순 방향성 베팅에서 복잡한 헤지 전략까지 구현 가능
- **리스크 감소**: 마켓 뉴트럴, 페어 트레이딩 등으로 시장 리스크 헤지
- **수익 안정성**: 장기 투자 및 리밸런싱으로 안정적 수익 추구
- **확장성**: 새로운 퀀트 전략을 쉽게 추가할 수 있는 구조
