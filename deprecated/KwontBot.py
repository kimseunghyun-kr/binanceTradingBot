...
import os
import sys
from analysis.analyzeData import run_weekly_analysis, run_daily_analysis
from config import ANALYSIS_SYMBOLS
from deprecated.symbols import initialize_symbols
from deprecated.backtest import run_backtesting, plot_grid_search_results, clear_signals_cache
        ...
        for current_add_buy_pct_val in add_buy_pcts:
            ...
            for tp_r_val in tp_ratios_list:
                for sl_r_val in sl_ratios_list:
                    ...
                    results = run_backtesting(tf, tp_r_val, sl_r_val, save_charts, current_add_buy_pct_val,
                                              use_cache=use_cache)
                    ...
                    if run_analysis:
                        ...
                        try:
                            success = analyze_single_add_buy_folder(target_directory,
                                                                    create_charts=create_position_charts)
                            if success:
                                    ...
                                    if os.path.exists(results_path):
                                        with open(results_path, 'r', encoding='utf-8') as f:
                                            ...
                                            if match:
                                                ...
                                                logging.info(
                                                    f"Using portfolio analysis return: {annual_return_pct:.2f}% (vs backtest: {results['total_return_pct']:.2f}%)")
                                except Exception as e:
                                    logging.warning(f"Could not extract annual return from analysis: {e}")

                                logging.info(
                                    f"✓ Portfolio analysis complete for TP={tp_folder}, SL={sl_folder}, Add.Buy={current_add_buy_pct_val:.1f}%")
                            else:
                                logging.warning(
                                    f"✗ Portfolio analysis failed for TP={tp_folder}, SL={sl_folder}, Add.Buy={current_add_buy_pct_val:.1f}%")
                        except Exception as e:
                        ...
                        if tp_sl_grid_choice.startswith("y"):
                            logging.info(
                                f"Portfolio analysis disabled - using backtest return: {results['total_return_pct']:.2f}%")
            ...
            if tp_sl_grid_choice.startswith("y") and grid_results_for_tp_sl:
                ...
                plot_grid_search_results(grid_results_for_tp_sl, tf, tp_sl_grid_plot_dir,
                                         add_buy_val_for_title=current_add_buy_pct_val)
