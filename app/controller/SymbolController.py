...
def get_binance_symbols():
    ...
    """
    symbols = SymbolService.get_binance_trading_symbols_df()
    if not symbols:
...
def filter_symbols_by_cap(min_cap: float = Query(150_000_000, description="Minimum market cap (USD)"),
    ...
    """
    filtered = SymbolService.get_symbols_by_market_cap(min_cap=min_cap, max_cap=max_cap, max_pages=max_pages)
